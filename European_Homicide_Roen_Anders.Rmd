...
Here I use the rename function to label the column Homicide. I then assign it to the dataset so it is visible in the digital object Western_Europe.

```{r How to rename the column}
Western_Europe %>% 
  rename(Homicide = `Homicide rate in Europe over long-term (per 100,000) (homicides per 100,000 people)`) -> Western_Europe
```
...
Here I think that a geom_area makes more sense, so I changed it to that:

```{r Updated geom_area}
ggplot(data = Western_Europe) + 
  geom_area(mapping = aes(x = Year, 
                           y = Western_Europe$Homicide,
                          fill = Entity,
                           color = Entity)) +
  facet_wrap(~ Entity, nrow = 3) +
...
I added the fill line, to fill up the area with color.

3) Can you change the facet layout to two columns and three rows so that the trends are more visible? 

## Task3

To me it seems that there are already two columns and three rows in the facet?

4) Move the legend from the current position on the side to below the facets, and label it "Country" instead of "Entity".

## Task4

Here, I used the legend.position function to put the legend on the bottom. Then I specified what the color and fill should be called - "Countries".
